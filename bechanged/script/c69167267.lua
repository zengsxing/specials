...
function c69167267.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if g:GetFirst():IsCode(73580471) then
...
function c69167267.spfilter2(c,e,tp)
	if not (c:IsFaceupEx() and aux.IsCodeOrListed(c,73580471) and c:IsType(TYPE_MONSTER)) then return false end	
...
function c69167267.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c69167267.spfilter2,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
