...
function c68300121.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,nil,4,2,c68300121.ovfilter,aux.Stringid(68300121,0),2,c68300121.xyzop)
	c:EnableReviveLimit()
	--to hand
...
function c68300121.ovfilter(c)
	return c:IsFaceup() and (c:IsSetCard(0x165) or (c:IsType(TYPE_NORMAL) and c:IsLevel(4)))
end
function c68300121.xyzop(e,tp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,68300121)==0 end
	Duel.RegisterFlagEffect(tp,68300121,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
...
function c68300121.rmcon(e,tp,eg,ep,ev,re,r,rp)
	...
	e:SetLabelObject(tc)
	return tc~=nil and Duel.IsExistingMatchingCard(c68300121.attrchkfilter,tp,LOCATION_GRAVE,0,1,nil,tc:GetAttribute())
end
