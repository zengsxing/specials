...
function c7548747.filter(c,tp,check)
	return c:IsFaceup() and ((c:IsSetCard(0x150) and c:IsControler(tp)) or c:IsControler(1-tp))
		and Duel.IsExistingMatchingCard(c7548747.eqfilter,tp,LOCATION_EXTRA+LOCATION_MZONE+LOCATION_GRAVE,0,1,c,check)
end
...
function c7548747.checkfilter(c)
	return c:IsSetCard(0x150) and c:IsType(0x1)
end
function c7548747.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ckg=Duel.GetMatchingGroup(c7548747.checkfilter,tp,LOCATION_GRAVE,0,nil)
	local check=(ckg:GetClassCount(Card.GetCode)>=3)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c7548747.filter(chkc,tp,check) end
	...
	if chk==0 then return ft>0
		and Duel.IsExistingTarget(c7548747.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,tp,check) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c7548747.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil,tp,check)
