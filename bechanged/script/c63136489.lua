...
function s.check(tp,g,fc)
	return g:IsExists(Card.IsRace,1,nil,RACE_BEAST+RACE_FIEND+RACE_ILLUSION)
end
...
function s.sfilter(c,e,tp)
	return (c:IsCode(5818798,77207191) or c:IsCode(64631466,78642798)) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end
...
function s.banish(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local sg=g:SelectSubGroup(tp,function (tg)
		return aux.dncheck(tg) and (tg:CheckWithSumEqual(s.bcheck,2,2,2) or tg:CheckWithSumGreater(s.bcheck,4,2,2))
	end,false,2,2)
	if sg then Duel.SpecialSummon(sg,0,tp,tp,true,true,POS_FACEUP) end
end
function s.bcheck(c)
	if c:IsCode(5818798,77207191) then return 1 end
	if c:IsCode(64631466,78642798) then return 2 end
end