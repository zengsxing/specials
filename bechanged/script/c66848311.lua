...
function s.controp(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsType(TYPE_MONSTER) then
		...
		local op=aux.SelectFromOptions(tp,{b1,aux.Stringid(id,1)},{true,aux.Stringid(id,3)})
		local ck=0
		if op==1 then
			if Duel.GetControl(tc,tp)>0 and tc:IsLocation(LOCATION_MZONE) then ck=1 end
		elseif op==2 then
			if Duel.Destroy(tc,0x40) then ck=1 end
		end
		if ck==1 and Duel.GetMatchingGroupCount(Card.IsFaceup,tp,0,LOCATION_MZONE,nil)>0
