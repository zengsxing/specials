...
function s.RitualCheck(g,lv)
	return g:GetSum(Card.GetLevel)==lv
end
function s.RitualCheckAdditional(lv)
	return	function(g)
				return g:GetSum(Card.GetLevel)<=lv
			end
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if b1 and (not b2 or Duel.SelectYesNo(tp,aux.Stringid(id,0))) then
		...
		local tc=sg:Filter(s.rfilter,nil,mc):SelectUnselect(nil,tp,false,true,1,1)
		...
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		aux.GCheckAdditional=s.RitualCheckAdditional(lv)
		local tg=sg:SelectSubGroup(tp,s.RitualCheck,true,1,ft,lv)
