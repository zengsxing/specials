...
function c3972721.initial_effect(c)
	...
	e0:SetDescription(aux.Stringid(3972721,0))
	e0:SetType(EFFECT_TYPE_ACTIVATE)
	e0:SetCategory(CATEGORY_DRAW)
	e0:SetCode(EVENT_FREE_CHAIN)
	e0:SetTarget(c3972721.target1)
	e0:SetOperation(c3972721.activate1)
	c:RegisterEffect(e0)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(3972721,1))
...
function c3972721.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) and Duel.IsPlayerCanDraw(1-tp,1) end
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,PLAYER_ALL,1)
end
function c3972721.activate1(e,tp,eg,ep,ev,re,r,rp)
