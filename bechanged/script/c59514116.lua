...
function c59514116.rsfilter(c,ras,e,tp,mg3,na,f,ca)
	if not (c:IsRace(RACE_SPELLCASTER) and c:IsAttribute(ATTRIBUTE_DARK))
		and c:IsLocation(LOCATION_DECK+LOCATION_GRAVE) then return false end
	return aux.RitualUltimateFilter(c,aux.TRUE,e,tp,mg3,nil,Card.GetLevel,"Greater")
end
function c59514116.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local res2=mg3:IsExists(Card.IsCode,1,nil,46986414,38033121)
		and Duel.IsExistingMatchingCard(c59514116.rsfilter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_DECK,0,1,nil,aux.TRUE,e,tp,mg3,nil,Card.GetLevel,"Greater")
...
function c59514116.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif e:GetLabel()==1 then
		...
		aux.RCheckAdditional=c59514116.rcheck
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local tg=Duel.SelectMatchingCard(tp,c59514116.rsfilter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,aux.TRUE,e,tp,mg,nil,Card.GetLevel,"Greater")
		local tc=tg:GetFirst()
		if tc then
