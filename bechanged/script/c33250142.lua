...
function c33250142.initial_effect(c)
	...
	e3:SetCondition(c33250142.thcon1)
	c:RegisterEffect(e3)
	--to hand (remove)
...
function c33250142.thfilter1(c)
	return c:IsSetCard(0x163,0x154) and c:IsAbleToHand()
end
...
function c33250142.thfilter2(c)
	return c33250142.thfilter1(c) and c:IsFaceup() and c:IsAbleToHand()
end
function c33250142.thtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) and chkc:IsControler(tp) and c33250142.thfilter2(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c33250142.thfilter2,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,aux.NecroValleyFilter(c33250142.thfilter2),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil)
