...
function s.matfilter1(c,syncard)
	return (c:IsTuner(syncard) and c:IsSetCard(0xaf)) or c:IsSetCard(0x10af)
end
function s.matfilter2(c,syncard)
	return c:IsNotTuner(syncard) and c:IsSetCard(0xaf)
end
...
function s.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not eg:IsContains(e:GetHandler())
		and Duel.IsExistingMatchingCard(s.disfilter,tp,0,LOCATION_MZONE,1,nil) end
end
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,1-tp,aux.Stringid(id,2))
	local pc=Duel.SelectMatchingCard(1-tp,s.disfilter,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
