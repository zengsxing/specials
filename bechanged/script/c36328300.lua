...
function c36328300.confilter(c,tp)
	return c:IsPreviousSetCard(0x1034) and c:IsPreviousControler(tp)
		and ((e:GetHandler():IsLocation(LOCATION_HAND) and Duel.GetMatchingGroupCount(function(tc) return tc:IsAttribute(ATTRIBUTE_DARK) and tc:IsFaceup() end,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)==0) or not e:GetHandler():IsLocation(LOCATION_HAND))
end
