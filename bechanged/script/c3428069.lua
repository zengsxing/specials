...
function c3428069.initial_effect(c)
	...
	e1:SetCondition(c3428069.spcon)
	e1:SetOperation(c3428069.spop)
	c:RegisterEffect(e1)
	--Equip
...
function c3428069.cfilter(c)
	return c:IsFaceup() and c:IsRace(RACE_DRAGON) and c:GetBaseAttack()<c:GetAttack()
end
function c3428069.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c3428069.cfilter,tp,0,LOCATION_MZONE,1,nil)
end
function c3428069.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_OATH)
	e1:SetTargetRange(1,0)
	e1:SetTarget(c3428069.splimit)
	e1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1,tp)
end
function c3428069.splimit(e,c)
