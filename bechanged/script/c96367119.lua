...
function c96367119.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(96367119,3))
	e3:SetType(EFFECT_TYPE_SINGLE)
	e3:SetCode(EFFECT_TRAP_ACT_IN_HAND)
	e3:SetCondition(c96367119.handcon)
	c:RegisterEffect(e3)
end
function c96367119.discon(e,tp,eg,ep,ev,re,r,rp)
	return re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
function c96367119.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x33) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
function c96367119.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c96367119.cfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c96367119.cfilter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c96367119.cfilter2(c)
	return c:IsFaceup() and ((c:IsSetCard(0x33) and c:IsType(TYPE_SYNCHRO)) or c:IsCode(9012916))
end
function c96367119.handcon(e)
	return Duel.IsExistingMatchingCard(c96367119.cfilter2,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil)
end