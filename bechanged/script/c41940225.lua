...
function c41940225.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON+CATEGORY_DECKDES+CATEGORY_SPECIAL_SUMMON)
...
function c41940225.spfilter(c,e,tp)
	return c:IsSetCard(0xd6) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,1-tp)
end
function c41940225.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return res or Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(c41940225.spfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp)
	end
end
function c41940225.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sp=Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(c41940225.spfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp)
	if res and (not sp or Duel.SelectOption(tp,aux.Stringid(41940225,0),aux.Stringid(41940225,2))==0) then
		local sg=sg1:Clone()
		...
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,c41940225.spfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,e,tp)
		local tc=g:GetFirst()
		if tc and Duel.SpecialSummonStep(tc,0,tp,1-tp,false,false,POS_FACEUP)~=0 then
...
function c41940225.eqlimit(e,c)
