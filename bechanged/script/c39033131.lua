...
function c39033131.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0 and ((not Duel.IsExistingMatchingCard(function (c) return not c:IsRace(RACE_ZOMBIE) and c:IsType(TYPE_MONSTER) end,tp,LOCATION_GRAVE,0,1,nil) and e:GetHandler():IsLocation(LOCATION_HAND)) or not e:GetHandler():IsLocation(LOCATION_HAND))
end
