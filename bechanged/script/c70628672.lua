...
function c70628672.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,0,rp,LOCATION_HAND+LOCATION_DECK+LOCATION_EXTRA)
end
function c70628672.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateActivation(ev) then
		local cd=re:GetHandler():GetCode()
		local g=Duel.GetMatchingGroup(Card.IsCode,rp,LOCATION_HAND+LOCATION_DECK+LOCATION_EXTRA,0,nil,cd)
