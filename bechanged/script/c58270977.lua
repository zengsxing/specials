...
function c58270977.initial_effect(c)
	...
	e1:SetCondition(function (e)
		...
		return ((e:GetHandler():IsLocation(LOCATION_HAND) and Duel.GetMatchingGroupCount(function(tc)
			return tc:GetOriginalType()&TYPE_MONSTER~=0 and tc:IsFaceup()
		end,tp,0,LOCATION_ONFIELD,nil)>0) or not e:GetHandler():IsLocation(LOCATION_HAND))
	end)
...
function c58270977.fselect(g,e,tp)
	return Duel.IsExistingMatchingCard(c58270977.spfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,g,e,tp) and Duel.GetMZoneCount(tp,g)>0
end
...
function c58270977.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c58270977.spfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,nil,e,tp) end
...
function c58270977.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c58270977.spfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,1,nil,e,tp):GetFirst()
	if tc and Duel.SpecialSummonStep(tc,0,tp,tp,true,false,POS_FACEUP) and e:GetLabel()==1 then
