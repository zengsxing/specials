...
function c34137269.initial_effect(c)
	...
	e7:SetCondition(c34137269.rtdcon)
	e7:SetTarget(c34137269.rtdtg)
	e7:SetOperation(c34137269.rtdop)
	c:RegisterEffect(e7)
end
function c34137269.rtdcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c34137269.rtdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,e:GetHandler(),1,0,0)
end
function c34137269.thfilter(c)
	return c:IsSetCard(0x4a) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c34137269.rtdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SendtoDeck(c,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)~=0
		and c:IsLocation(LOCATION_DECK)
		and Duel.IsExistingMatchingCard(c34137269.thfilter,tp,LOCATION_DECK,0,1,nil) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,c34137269.thfilter,tp,LOCATION_DECK,0,1,1,nil)
