...
function c84673417.initial_effect(c)
	...
	e3:SetCountLimit(1,84673417)
	e3:SetCost(c84673417.spcost)
	e3:SetTarget(c84673417.sptg)
	e3:SetOperation(c84673417.spop)
	c:RegisterEffect(e3)
end
function c84673417.cfilter(c)
	return c:IsType(0x1) and c:IsSetCard(0x38) and c:IsAbleToRemoveAsCost()
end
function c84673417.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c84673417.cfilter,tp,LOCATION_GRAVE,0,1,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c84673417.cfilter,tp,LOCATION_GRAVE,0,1,1,c)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c84673417.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c84673417.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
