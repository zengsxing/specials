...
function c98552723.initial_effect(c)
	...
	e5:SetDescription(aux.Stringid(98552723,1))
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetRange(LOCATION_GRAVE)
	e5:SetCost(c98552723.thcost)
	e5:SetTarget(c98552723.thtg)
	e5:SetOperation(c98552723.thop)
	c:RegisterEffect(e5)
end
...
function c98552723.thfilter(c)
	return c:IsAttack(800) and c:IsDefense(1000) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
end
function c98552723.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c98552723.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c98552723.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c98552723.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c98552723.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
