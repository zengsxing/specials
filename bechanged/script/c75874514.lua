...
function s.filter(c,e,tp)
	if Duel.GetTurnPlayer()==tp then
		return (c:IsSetCard(0xc2) or c:IsRace(RACE_DRAGON) and c:IsLevel(7,8)) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
	else
		return (c:IsSetCard(0xc2) or c:IsRace(RACE_DRAGON)) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
	end
end
...
function s.thfilter(c)
	return not c:IsCode(id) and aux.IsCodeListed(c,63436931) and c:IsAbleToHand()
end
