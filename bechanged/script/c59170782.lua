...
function c59170782.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,nil,3,3,c59170782.ovfilter,aux.Stringid(59170782,2),3,c59170782.xyzop)
	c:EnableReviveLimit()
	--change target
...
function c59170782.ovfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x74) and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_XYZ)
end
function c59170782.xyzop(e,tp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,59170782)==0 end
	Duel.RegisterFlagEffect(tp,59170782,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
