...
function c46724542.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(46724542,0))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SPSUMMON_SUCCESS)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	e1:SetCountLimit(1,46724543)
	e1:SetTarget(c46724542.thtg)
	e1:SetOperation(c46724542.thop)
	c:RegisterEffect(e1)
	--atk/def
...
function c46724542.tffilter(c,tp)
	return c:IsCode(33099732) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function c46724542.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local ft=Duel.GetLocationCount(tp,LOCATION_SZONE)
		return ft>0 and Duel.IsExistingMatchingCard(c46724542.tffilter,tp,LOCATION_DECK,0,1,nil,tp)
	end
end
function c46724542.thop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,c46724542.tffilter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
	if tc then
