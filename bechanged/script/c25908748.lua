...
function c25908748.initial_effect(c)
	...
	e22:SetDescription(aux.Stringid(25908748,0))
	e22:SetType(EFFECT_TYPE_FIELD)
	e22:SetCode(EFFECT_EXTRA_SUMMON_COUNT)
	e22:SetRange(LOCATION_SZONE)
	e22:SetTargetRange(LOCATION_HAND+LOCATION_MZONE,0)
	e22:SetTarget(aux.TargetBoolFunction(Card.IsRace,RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINDBEAST))
...
function c25908748.cfilter(c,tp)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and c:IsRace(RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINDBEAST)
...
function c25908748.srfilter(c,race)
	return c:IsRace(RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINDBEAST) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:GetOriginalRace()~=race
end
...
function c25908748.srop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if not tc:IsSetCard(0x14d) then
			...
			e1:SetTarget(c25908748.sumlimit)
...
function c25908748.sumlimit(e,c)
