...
function c1621413.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,nil,5,3,c1621413.ovfilter,aux.Stringid(1621413,0),3,c1621413.xyzop)
	c:EnableReviveLimit()
	--atk
...
function c1621413.cfilter(c)
	return (c:IsSetCard(0x95) and c:IsType(TYPE_SPELL)
		or c:IsSetCard(0x10db) and c:IsType(TYPE_SPELL+TYPE_TRAP))
		and c:IsDiscardable()
end
function c1621413.ovfilter(c)
	return c:IsFaceup() and c:IsCode(16195942)
end
function c1621413.xyzop(e,tp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1621413.cfilter,tp,LOCATION_HAND,0,1,nil) and Duel.GetFlagEffect(tp,1621413)==0 end
	Duel.DiscardHand(tp,c1621413.cfilter,1,1,REASON_COST+REASON_DISCARD)
	Duel.RegisterFlagEffect(tp,1621413,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
