...
function c90299015.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND+CATEGORY_TOGRAVE+CATEGORY_SPECIAL_SUMMON)
...
function c90299015.filter1(c,e,tp)
	return c:IsSetCard(0x8e) and c:IsLevelAbove(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c90299015.filter2(c)
	return c:IsSetCard(0x8e) and c:IsLevelBelow(2) and c:IsAbleToGrave()
end
function c90299015.filter3(c)
	return c:IsSetCard(0x8e) and c:IsType(0x6) and c:IsAbleToGrave()
end
function c90299015.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c90299015.filter1,tp,LOCATION_DECK,0,1,nil,e,tp)
		and Duel.IsExistingMatchingCard(c90299015.filter2,tp,LOCATION_DECK,0,1,nil)
		and Duel.IsExistingMatchingCard(c90299015.filter3,tp,LOCATION_DECK,0,1,nil) end
...
function c90299015.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,c90299015.filter1,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g1:GetCount()>0 and Duel.SpecialSummon(g1,0,tp,tp,false,false,POS_FACEUP)>0 then
		...
		if g2:GetCount()>0 and Duel.SendtoGrave(g2,REASON_EFFECT)>0 and g2:GetFirst():IsLocation(LOCATION_GRAVE) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g3=Duel.SelectMatchingCard(tp,c90299015.filter3,tp,LOCATION_DECK,0,1,1,nil)
			if g3:GetCount()>0 then
				Duel.SendtoHand(g3,nil,0x40)
