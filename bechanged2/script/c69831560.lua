...
function c69831560.spfilter(c,tp)
	return c:IsAbleToGraveAsCost() and (c:IsFaceup() or c:IsControler(tp))
end
function c69831560.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local mg=Duel.GetMatchingGroup(c69831560.spfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,tp)
	return mg:CheckSubGroup(aux.mzctcheck,3,3,tp)
end
function c69831560.sptg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local mg=Duel.GetMatchingGroup(c69831560.filter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,tp)
