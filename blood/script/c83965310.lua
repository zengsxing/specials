...
function c83965310.initial_effect(c)
	...
	e6:SetDescription(aux.Stringid(83965310,0))
	e6:SetCategory(CATEGORY_REMOVE)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_SPSUMMON_SUCCESS)
	e6:SetProperty(EFFECT_FLAG_DELAY)
	e6:SetTarget(c83965310.target)
	e6:SetOperation(c83965310.operation)
	c:RegisterEffect(e6)
	--immune quick spell
...
function c83965310.tgfilter(c)
	return c:IsAbleToRemove()
end
function c83965310.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(c83965310.tgfilter,tp,LOCATION_DECK,0,1,nil)
	end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_DECK)
end
function c83965310.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c83965310.tgfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c83965310.efilter(e,te)
	local c=te:GetHandler()
	return c:GetType()==(TYPE_SPELL+TYPE_QUICKPLAY)
end