...
function CHEST.EffectMessageAbsolute(e,rp)
    ...
    local g=Duel.GetMatchingGroup(CHEST.filter,rp,LOCATION_DECK,0,nil,e,rp)
    if #g<1 or Duel.GetMZoneCount(rp)<1 then
        return "但是它好像坏掉了……"
    end
    return name .. "使用了召唤解限器！"
end
...
function CHEST.BattleDestroyedEffect(e,rp)
    local g=Duel.GetMatchingGroup(CHEST.filter,rp,LOCATION_DECK,0,nil,e,rp):RandomSelect(rp,1)
    Duel.SpecialSummon(g,0,rp,rp,true,true,POS_FACEUP)
end