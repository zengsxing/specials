...
function c58984738.initial_effect(c)
	...
	e3:SetTarget(c58984738.settarget)
	e3:SetOperation(c58984738.setoperation)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
...
function c58984738.pfilter(c,tp)
	return c:GetType()==0x20004 and c:IsSetCard(0xf9)
		and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function c58984738.settarget(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c58984738.pfilter,tp,LOCATION_GRAVE,0,1,nil,tp) end
end
function c58984738.setoperation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,c58984738.pfilter,tp,LOCATION_GRAVE,0,1,1,nil,tp):GetFirst()
