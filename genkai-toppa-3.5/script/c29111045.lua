...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,s.costfilter,2,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,s.costfilter,2,2,nil,tp)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,2,nil,e,tp) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(g) do
		if tc and Duel.SpecialSummonStep(g,0,tp,tp,false,false,POS_FACEUP)~=0 then
