...
function s.dacon(e)
	return Duel.GetOverlayCount(0,1,1)>=6
end
function s.filter(c,e)
	return c:IsCanChangePosition() and c:IsCanBeEffectTarget(e) and c:IsLocation(LOCATION_MZONE)
end
function s.postg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(s.filter,2,nil,e) and not eg:IsContains(e:GetHandler()) and Duel.CheckRemoveOverlayCard(tp,1,1,2,REASON_EFFECT) end
	local tc=eg:FilterSelect(tp,s.filter,2,2,nil,e)
