...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0xac) and c:IsType(TYPE_MONSTER) and c:IsLevelBelow(8) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp)
			and Duel.CheckRemoveOverlayCard(tp,1,1,2,REASON_EFFECT)
			and Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(id,2)) then
			Duel.BreakEffect()
			Duel.RemoveOverlayCard(tp,1,1,2,2,REASON_EFFECT)
			local sg=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND,0,2,2,nil,e,tp)
	...
	if Duel.CheckRemoveOverlayCard(tp,1,1,2,REASON_EFFECT) then
		...
		if Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_GRAVE,0,1,nil) then
			if Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(id,3)) then
				local cg=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_GRAVE,0,2,2,nil)
