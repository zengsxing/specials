...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:GetHandler():IsCanOverlay()
		and Duel.IsExistingTarget(s.filter,tp,LOCATION_MZONE,0,2,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,s.filter,tp,LOCATION_MZONE,0,2,2,nil,e,tp)
...
function s.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.mfilter,tp,LOCATION_MZONE,0,1,nil,e,tp) and Duel.IsExistingTarget(s.ofilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,2,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g1=Duel.SelectTarget(tp,s.mfilter,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
	local g2=Duel.SelectTarget(tp,s.ofilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,2,2,nil)
...
function s.matop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc2=tg:Filter(Card.IsLocation,nil,LOCATION_GRAVE)
